...
class CrackDataset(BaseDataset):
    ...
    def __init__(self, args):
        ...
        self.img_paths = make_dataset(os.path.join(args.dataset_path, 'images'))
        self.lab_dir = os.path.join(args.dataset_path, 'masks')
    ...
    def __getitem__(self, index):
        ...
        if lab is None:
            raise ValueError(f"Could not read label: {lab_path}")
        if len(lab.shape) == 3:

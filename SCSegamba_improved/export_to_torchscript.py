...
OUTPUT_PT_PATH = Path("torchscript_exports/TUT_eca_dynamic.pt")
...
# === Wrap model for grayscale (1→3) expansion ===
...
# === Trace with flexible size ===
example = torch.randn(1, 1, 512, 512, device=args.device)
#traced = torch.jit.script(wrapper, example)
#traced.save(str(OUTPUT_PT_PATH))
print(f"[✓] Exported TorchScript model to {OUTPUT_PT_PATH}")

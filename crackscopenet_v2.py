...
from paddleseg.models.backbones.transformer_utils import (DropPath, ones_, to_2tuple, zeros_)
from paddleseg.models import layers

def get_depthwise_conv(dim):
    return nn.Conv2D(
        in_channels=dim,
        out_channels=dim,
        kernel_size=3,
...
class VortexPooling(nn.Layer):
    def __init__(self, in_channels, out_channels):
        super(VortexPooling, self).__init__()
        self.avgpool_3 = nn.AvgPool2D(3, stride=1, padding=1)
        self.avgpool_5 = nn.AvgPool2D(5, stride=1, padding=2)
        self.avgpool_7 = nn.AvgPool2D(7, stride=1, padding=3)
    ...
    def forward(self, x):
        x1 = self.conv_1x1(x)
        x2 = self.conv_3x3(self.avgpool_3(x))
        x3 = self.conv_3x3(self.avgpool_5(x))
        x4 = self.conv_5x5(self.avgpool_7(x))
        out = paddle.concat([x1, x2, x3, x4], axis=1)
        out = self.fuse(out)
        return self.relu(out)

class MS_CascadedASPP(nn.Layer):
    def __init__(self, in_channels, out_channels):
        super(MS_CascadedASPP, self).__init__()
        # First ASPP
        self.conv1_1 = nn.Conv2D(in_channels, out_channels, 1)
        self.conv1_3 = nn.Conv2D(in_channels, out_channels, 3, padding=1, dilation=1)
        self.conv1_6 = nn.Conv2D(in_channels, out_channels, 3, padding=6, dilation=6)
        self.conv1_12 = nn.Conv2D(in_channels, out_channels, 3, padding=12, dilation=12)
        # Second, cascaded ASPP
        self.conv2_1 = nn.Conv2D(out_channels*4, out_channels, 1)
        self.conv2_3 = nn.Conv2D(out_channels*4, out_channels, 3, padding=1, dilation=1)
        self.conv2_6 = nn.Conv2D(out_channels*4, out_channels, 3, padding=6, dilation=6)
        self.conv2_12 = nn.Conv2D(out_channels*4, out_channels, 3, padding=12, dilation=12)
        self.fuse = nn.Conv2D(out_channels*4, out_channels, 1)
        self.relu = nn.ReLU()
    def forward(self, x):
        aspp1 = [
        ...
        aspp1_out = paddle.concat(aspp1, axis=1)
        aspp2 = [
            self.conv2_1(aspp1_out),
            self.conv2_3(aspp1_out),
            self.conv2_6(aspp1_out),
            self.conv2_12(aspp1_out)
        ]
        out = paddle.concat(aspp2, axis=1)
        out = self.fuse(out)
        return self.relu(out)

# --------- (Rest of the unchanged classes: BasicConv, SWA, Mlp, CSModule, etc.) ---------
# (Paste in all your unchanged model classes here.)
...
# --------- MAIN CRACKSCOPENET ARCHITECTURE WITH CONTEXT MODULE ----------
...
class CrackScopeNet_Seg(nn.Layer):
    def __init__(
            ...
            context_module: Optional[nn.Layer] = None   # Can be None, VortexPooling, or MSC-ASPP
    ):
        super(CrackScopeNet_Seg, self).__init__()
        self.pretrained = pretrained
        self.feat_channels = feat_channels
        self.stem = Stem(3, base)
        self.stage1 = CrackStage(feat_channels[0], feat_channels[0], block_num[0], stage_num=1, drop_path=dropout_rate)
        self.stage2 = CrackStage(feat_channels[0], feat_channels[1], block_num[1], drop_path=dropout_rate)
        self.stage3 = CrackStage(feat_channels[1], feat_channels[2], block_num[2], drop_path=dropout_rate)

        self.context_module = context_module  # Will be None, VortexPooling, or MSC-ASPP

        self.up2 = UpsamplingBottleneck(feat_channels[2], feat_channels[1])
        ...
        self.cat = CSModule_Upsampling(feat_channels[0]*2, feat_channels[0]*2)
        self.dw3 = DWConv(feat_channels[0]*2, feat_channels[0]*2)
        self.seghead = AuxSegHead(feat_channels[0]*2, feat_channels[0], num_classes)
        self.aux_head1 = AuxSegHead(feat_channels[2], feat_channels[2]//2, num_classes)
        self.aux_head2 = AuxSegHead(feat_channels[1], feat_channels[1]//2, num_classes)
        self.aux_head3 = AuxSegHead(feat_channels[0], feat_channels[0]//2, num_classes)
        self.aux_head4 = AuxSegHead(feat_channels[0]*2, feat_channels[0], num_classes)
        self.init_weight()
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            up2 = self.up2(feat3)
            feat2_resized = F.interpolate(feat2, size=up2.shape[2:], mode='bilinear', align_corners=True)
            up3 = self.up3(self.dw3_1(self.cat1(paddle.concat([up2, feat2_resized], axis=1))))

            feat1_resized = F.interpolate(feat1, size=up3.shape[2:], mode='bilinear', align_corners=True)
            cat = self.dw3(self.cat(paddle.concat([up3, feat1_resized], axis=1)))
    ...
    def init_weight(self):
        if self.pretrained is not None:
            print("Loading weight from pretrained model")
...
# ----------- MODEL SELECTION FUNCTIONS (PaddleSeg Components) ------------
...
def CrackScopeNet_Vortex(num_classes=2):
    return CrackScopeNet_Seg(32, (3, 3, 4), (32, 64, 128), dropout_rate=0.1, num_classes=num_classes,
        context_module=VortexPooling(128, 128))

@manager.MODELS.add_component
def CrackScopeNet_MSCASPP(num_classes=2):
    return CrackScopeNet_Seg(32, (3, 3, 4), (32, 64, 128), dropout_rate=0.1, num_classes=num_classes,
        context_module=MS_CascadedASPP(128, 128))

@manager.MODELS.add_component
def CrackScopeNet_Large_Vortex(num_classes=2):
    return CrackScopeNet_Seg(64, (3, 3, 3), (64, 128, 160), dropout_rate=0.1, num_classes=num_classes,
        context_module=VortexPooling(160, 160))

@manager.MODELS.add_component
def CrackScopeNet_Large_MSCASPP(num_classes=2):
    return CrackScopeNet_Seg(64, (3, 3, 3), (64, 128, 160), dropout_rate=0.1, num_classes=num_classes,
        context_module=MS_CascadedASPP(160, 160))

# -------------------------------------------------------------------------

# Note: Be sure to keep all the required unchanged model building blocks (BasicConv, SWA, etc.) in this file.
